...
### 环境变量的设置
环境变量一般指的是用export内置命令导出的变量，用于定义shell的运行环境、保证shell命令的正确执行。shell通过环境变量确定登录的用户名、PATH路径、文件
系统等各种应用。
...
每个用户都有自己的环境变量配置文件，~/.bash_profile ~/.bashrc，且以个人配置文件，优先加载变量，读取，以个人的优先生效。当你需要给所有的用户都使用
某个变量，写入全局即可/etc/profile
