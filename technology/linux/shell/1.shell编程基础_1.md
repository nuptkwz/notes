...
```shell
[root@kwz shell]# /bin/bash ./hello.sh
hello world
```
...
```shell
[root@kwz shell]# /bin/sh ./hello.sh
hello world
```
...
```shell
[root@kwz shell]# source hello.sh
hello world
```
...
```shell
[root@kwz shell]# history -c
[root@kwz shell]# history
   44  history
   
[root@kwz shell]# history -r
[root@kwz shell]# history
...
```shell
[root@kwz shell]# name="keweizhou"
[root@kwz shell]# echo ${name}
keweizhou
# 也可以简写
[root@kwz shell]# echo $name
keweizhou
...
```shell
[root@kwz shell]# readonly title="kwz"
[root@kwz shell]# echo $title
kwz
[root@kwz shell]# title="hello"
...
执行结果：
[root@kwz shell]# bash special_var.sh a b c d e f
...
返回
[root@kwz shell]# sh special_state.sh a b
参数个数为2
当前脚本的id是:3478
```
...
```text
[root@kwz shell]# eval ls;sh hello.sh
...
#利用数值函数计算expr命令
[root@kwz shell]# expr length "${name}"
3

#awk统计长度，length函数
[root@kwz shell]# echo "${name}" | awk '{print length($0)}'
3
...
```shell
[root@kwz ~]# time for n in {1..1000};do char=`seq -s "hello" 100`;echo ${#char} &>/dev/null;done
...
```shell
[root@kwz ~]# time for n in {1..1000};do char=`seq -s "hello" 100`;echo ${char}|wc -L  &>/dev/null;done
...
```shell
[root@kwz ~]# time for n in {1..1000};do char=`seq -s "hello" 100`;expr length ${char}  &>/dev/null;done
...
```shell
[root@kwz ~]# time for n in {1..1000};do char=`seq -s "hello" 100`;echo ${char}|awk '{print length($s0)}'  &>/dev/null;done
...
```shell
[root@kwz ~]# echo ${name:3:5}
hello

[root@kwz ~]# name2="abcABC123ABCabc"
[root@kwz ~]# echo ${name2#a*c}
ABC123ABCabc
[root@kwz ~]#
[root@kwz ~]# echo ${name2##a*c}
...
```shell
多次匹配替换
[root@kwz ~]# str1="hello, Jake! i am joke"
[root@kwz ~]# echo ${str1}
hello, Jake! i am joke
[root@kwz ~]# echo ${str1/o/Q}
...
```shell
[root@kwz nginx_log]# f=nginx_2_finished.log
[root@kwz nginx_log]#
[root@kwz nginx_log]# echo ${f}
nginx_2_finished.log
[root@kwz nginx_log]# echo ${f//_finished/}
nginx_2.log
```
...
```shell
[root@kwz nginx_log]# mv $f ${f//_finished/}
[root@kwz nginx_log]#
[root@kwz nginx_log]# ll
total 0
-rw-r--r-- 1 root root 0 Aug 20 12:56 nginx_1_finished.text
...
```shell
[root@kwz shell]# echo ${parameter}

[root@kwz shell]# echo ${word}
123
[root@kwz shell]# result=${parameter:-word}
[root@kwz shell]# echo ${result}
word
[root@kwz shell]#
...
[root@kwz shell]# echo ${word}
123
[root@kwz shell]# res=${parameter:=word}
[root@kwz shell]# echo ${res}
word
[root@kwz shell]# echo ${parameter}
word
[root@kwz shell]# echo ${word}
...
[root@kwz shell]# echo ${parameter:?变量值为空}
...
[root@kwz shell]# parameter=keweizhou
[root@kwz shell]# echo ${parameter:?变量值为空}
keweizhou
[root@kwz shell]#
```
...
[root@kwz shell]# word="test"
[root@kwz shell]# echo ${word}
test
[root@kwz shell]# echo ${parameter:+word}
...
```shell
[root@kwz shell]# parameter="hello"
[root@kwz shell]# echo ${word}
test
[root@kwz shell]# echo ${parameter:+word}
word
